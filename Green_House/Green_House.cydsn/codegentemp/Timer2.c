...
void Timer2_Init(void) 
{
        ...
        /* Check if One Shot mode is enabled i.e. RunMode !=0*/
            ...
            Timer2_CONTROL |= 0x04u;
        #endif /* One Shot enabled only when RunModeUsed is not Continuous*/

        #if (Timer2_RunModeUsed == 2)
            #if (CY_PSOC5A)
                /* Set last 2 bits of control2 register if one shot(halt on
                interrupt) is enabled*/
                Timer2_CONTROL2 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Set last 2 bits of control3 register if one shot(halt on
                interrupt) is enabled*/
                Timer2_CONTROL3 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL3 for PSoC3 or PSoC5LP */

        #endif /* Remove section if One Shot Halt on Interrupt is not enabled */

        #if (Timer2_UsingHWEnable != 0)
            #if (CY_PSOC5A)
                /* Set the default Run Mode of the Timer to Continuous */
                Timer2_CONTROL2 |= Timer2_CTRL_MODE_PULSEWIDTH;
            #endif /* Set Continuous Run Mode in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Clear and Set ROD and COD bits of CFG2 register */
                Timer2_CONTROL3 &= ((uint8)(~Timer2_CTRL_RCOD_MASK));
                Timer2_CONTROL3 |= Timer2_CTRL_RCOD;

                /* Clear and Enable the HW enable bit in CFG2 register */
                Timer2_CONTROL3 &= ((uint8)(~Timer2_CTRL_ENBL_MASK));
                Timer2_CONTROL3 |= Timer2_CTRL_ENBL;

                /* Set the default Run Mode of the Timer to Continuous */
                Timer2_CONTROL3 |= Timer2_CTRL_MODE_CONTINUOUS;
    ...
    Timer2_WritePeriod(Timer2_INIT_PERIOD);
    Timer2_WriteCounter(Timer2_INIT_PERIOD);

    #if (Timer2_UsingHWCaptureCounter)/* Capture counter is enabled */
        Timer2_CAPTURE_COUNT_CTRL |= Timer2_CNTR_ENABLE;
        Timer2_SetCaptureCount(Timer2_INIT_CAPTURE_COUNT);
    #endif /* Configure capture counter value */

    #if (!Timer2_UsingFixedFunction)
        #if (Timer2_SoftwareCaptureMode)
            Timer2_SetCaptureMode(Timer2_INIT_CAPTURE_MODE);
        #endif /* Set Capture Mode for UDB implementation if capture mode is software controlled */

        #if (Timer2_SoftwareTriggerMode)
            #if (!Timer2_UDB_CONTROL_REG_REMOVED)
                if (0u == (Timer2_CONTROL & Timer2__B_TIMER__TM_SOFTWARE))
                {
                    Timer2_SetTriggerMode(Timer2_INIT_TRIGGER_MODE);
                }
        ...
        CyExitCriticalSection(Timer2_interruptState);
            ...
            Timer2_EnableTrigger();
        #endif /* Set Trigger enable bit for UDB implementation in the control register*/
		
		
        #if (Timer2_InterruptOnCaptureCount && !Timer2_UDB_CONTROL_REG_REMOVED)
            Timer2_SetInterruptCount(Timer2_INIT_INT_CAPTURE_COUNT);
        #endif /* Set interrupt count in UDB implementation if interrupt count feature is checked.*/

        Timer2_ClearFIFO();
    #endif /* Configure additional features of UDB implementation */

    Timer2_SetInterruptMode(Timer2_INIT_INTERRUPT_MODE);
}
...
void Timer2_Enable(void) 
{
    ...
    #if (!Timer2_UDB_CONTROL_REG_REMOVED || Timer2_UsingFixedFunction)
        Timer2_CONTROL |= Timer2_CTRL_ENABLE;
...
{
    Timer2_STATUS_MASK = interruptMode;
}


/*******************************************************************************
* Function Name: Timer2_SoftwareCapture
********************************************************************************
*
* Summary:
...
#if (!Timer2_UDB_CONTROL_REG_REMOVED) /* Remove API if control register is unused */
...
{
   #if(Timer2_UsingFixedFunction)
       return ((uint32)CY_GET_REG16(Timer2_PERIOD_LSB_PTR));
   #else
       return (CY_GET_REG32(Timer2_PERIOD_LSB_PTR));
   #endif /* (Timer2_UsingFixedFunction) */
}
...
void Timer2_WritePeriod(uint32 period) 
{
        ...
        CY_SET_REG16(Timer2_PERIOD_LSB_PTR, period_temp);
    #else
        CY_SET_REG32(Timer2_PERIOD_LSB_PTR, period);
...
{
   #if(Timer2_UsingFixedFunction)
       return ((uint32)CY_GET_REG16(Timer2_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG32(Timer2_CAPTURE_LSB_PTR));
   #endif /* (Timer2_UsingFixedFunction) */
}
...
void Timer2_WriteCounter(uint32 counter) 
{
    ...
    #else
        CY_SET_REG32(Timer2_COUNTER_LSB_PTR, counter);
...
#if(!Timer2_UsingFixedFunction) /* UDB Specific Functions */
...
*  Timer2__B_TIMER__CM_NONE        - Set Capture mode to None
*  Timer2__B_TIMER__CM_RISINGEDGE  - Rising edge of Capture input
*  Timer2__B_TIMER__CM_FALLINGEDGE - Falling edge of Capture input
*  Timer2__B_TIMER__CM_EITHEREDGE  - Either edge of Capture input
*
* Return:
*  void
*
...
void Timer2_SetCaptureMode(uint8 captureMode) 
{
    ...
    captureMode = ((uint8)((uint8)captureMode << Timer2_CTRL_CAP_MODE_SHIFT));
    captureMode &= (Timer2_CTRL_CAP_MODE_MASK);

    #if (!Timer2_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Timer2_CONTROL &= ((uint8)(~Timer2_CTRL_CAP_MODE_MASK));

        /* Write The New Setting */
        Timer2_CONTROL |= captureMode;
    #endif /* (!Timer2_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: Timer2_SetTriggerMode
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    triggerMode &= Timer2_CTRL_TRIG_MODE_MASK;

    #if (!Timer2_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
    
        /* Clear the Current Setting */
        Timer2_CONTROL &= ((uint8)(~Timer2_CTRL_TRIG_MODE_MASK));

        /* Write The New Setting */
        Timer2_CONTROL |= (triggerMode | Timer2__B_TIMER__TM_SOFTWARE);
...
/*******************************************************************************
* Function Name: Timer2_EnableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!Timer2_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
        Timer2_CONTROL |= Timer2_CTRL_TRIG_EN;
    #endif /* Remove code section if control register is not used */
}


/*******************************************************************************
* Function Name: Timer2_DisableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!Timer2_UDB_CONTROL_REG_REMOVED )   /* Remove assignment if control register is removed */
        Timer2_CONTROL &= ((uint8)(~Timer2_CTRL_TRIG_EN));
    #endif /* Remove code section if control register is not used */
}
...
/*******************************************************************************
* Function Name: Timer2_SetInterruptCount
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    interruptCount &= Timer2_CTRL_INTCNT_MASK;

    #if (!Timer2_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Timer2_CONTROL &= ((uint8)(~Timer2_CTRL_INTCNT_MASK));
        /* Write The New Setting */
        Timer2_CONTROL |= interruptCount;
    #endif /* (!Timer2_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: Timer2_SetCaptureCount
********************************************************************************
*
* Summary:
...
{
    Timer2_CAP_COUNT = captureCount;
}


/*******************************************************************************
* Function Name: Timer2_ReadCaptureCount
********************************************************************************
*
* Summary:
